    ...
    router.post('/',(req,res)=>{
       //！！！！！！！！！！！ 
        ...
        if(newFileName){
            fs.rename(oldPath,newPath,(err)=>{
                if(err){
                    ...
                    if(req.body.mod_id){    //修改
                            ...
                            if(err){
                                ...
                                fs.unlink('public/upload/'+data[0].pic,(err)=>{
                                    if(err){
                                        ...
                                        id = '${id}',username='${username}',phone = '${phone}',password='${password}',email='${email}',introduction='${introduction}' \
                                        pic='${newFileName}' \
                        ...
                        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        db.query(`insert into login (id, pic,username,phone,password,email,introduction)
                        values('${id}','${newFileName}','${username}','${phone}','${password}','${email}','${introduction}')`,(err,data)=>{
                        if(err){
                            console.error(err);
            ...
            //!!!!!!!!!!!!!!!!!!!
                db.query(`update login set \
                 id = ${id},username='${username}',phone = '${phone}',password='${password}',email='${email}',qq='${qq}' \
                where id = ${req.body.mod_id}`,(err)=>{
                    if(err){
                        console.error(err);
